...
(function () {
  ...
  const DATABASE_NAME = "items-collection";
  const ITEM_NAME = "mod";
  const ID_FIELD = "id";
  ...
  const ITEM_FIELDS = [ID_FIELD, CATEGORY_FIELD, TITLE_FIELD, AUTHOR_FIELD, CREATED_AT_FIELD];
  const BASE_URL = "https://gamebanana.com";
  const MODS_ID_PREFIX = "mods/";
  const BACKUP_EXTENSION = ".json";
  const BACKUP_NAME = `${ITEM_NAME}-collection-backup${BACKUP_EXTENSION}`;
  ...
  const SHADOW_CLASS_NAME = "shadowy-item";
  ...
  function scrapeItem() {
    ...
    const author = document.querySelector("#SubmitterModule a.Avatar")?.querySelector("img[alt]")?.getAttribute("alt")?.replace(" avatar", "");
  ...
  function observeContent(target) {
  ...
  async function saveItem(item) {
    return db[ITEM_NAME].add(item).then((result) => {
      if (result) {
  ...
  async function deleteItem(itemId) {
    return db[ITEM_NAME].delete(itemId).then(() => console.info(`Item deleted: ${itemId}`)).catch(handleError);
  }
  async function getItem(id) {
    return db[ITEM_NAME].get(id).catch(handleError);
  }
  async function exportBackup() {
    db[ITEM_NAME].toArray().then((items) => {
  ...
  async function importBackup() {
    ...
    input.onchange = async () => {
      ...
      file.text().then((text) => {
        const items = JSON.parse(text, (key, value) => {
          if (key === CREATED_AT_FIELD) {
            return new Date(value);
          }
          return value;
        });
        db[ITEM_NAME].bulkAdd(items).then(() => console.info(`Items imported: ${items.length}`)).catch(handleError);
      }).catch(handleError);
    };
    input.click();
  }
